...
class Adaptive_CACCController:
    ...
    def calc_speed(self, leader_v, leader_a, front_dis, front_v, front_a, ego_v, ego_a,
        ...
        feedforward_acc = self.feedforward_gain * (leader_tv - ego_v) * 0.1
        feedforward_acc *= math.tanh(leader_a * leader_delay) * (leader_tv - leader_v)

        leading_v_err = leader_tv - ego_v
        spacing_err = front_dis - self.headway_time * ego_v - self.emergency_threshold + 0.2
        speed_err = front_v - ego_v
        ...
        output = (feedforward_acc +
                  ...
                  gap_control_gain_gap * self.estimated_spacing_err +
                  gap_control_gain_gap_dot * self.estimated_speed_err)

        if abs(ego_v - 15) < 0.2 and abs(leader_v - 15) < 0.2 and abs(spacing_err) < 0.3:
